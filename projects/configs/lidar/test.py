# _base_ = [
#     '../_base_/datasets/nuscenes_fusion.py',
...
data_root = 's3://e2emodel-data/e2emodel_dataset/nuscenes-mmdet3d/'
gt_aug_root = '/data/tr3d_pkl/'
input_modality = dict(
    use_lidar=True,
    use_camera=True,
    use_radar=False,
...
img_norm_cfg = dict(
    mean=[103.530, 116.280, 123.675], std=[57.375, 57.120, 58.395], to_rgb=True)
ida_aug_conf = {
...
train_pipeline = [
    dict(
        type='LoadPointsFromNori',
        coord_type='LIDAR',
        load_dim=5,
        use_dim=[0, 1, 2, 3, 4],
    ),
    dict(
        type='LoadPointsFromMultiSweepsNori',
        sweeps_num=10,
        use_dim=[0, 1, 2, 3, 4],
    ),
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            data_root=data_root,
            ann_file=data_root + 'nuscenes_infos_train_new.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_infos_val_new.pkl',
...
model = dict(
    ...
    pts_bbox_head=dict(
        type='CenterCMTLiDARHeadV1SparseLocalMaxSubConvV1InitDropToken',
        ...
        transformer=dict(
            type='TR3DLiDARTransformerSparseInit',
            decoder=dict(
...
log_level = 'INFO'
work_dir = None
# load_from='/home/zhangdiankun/fcos3d_vovnet_imgbackbone-remapped_perceptron.pth'
load_from = None
resume_from = None
workflow = [('train', 1)]
gpu_ids = range(0, 8)
