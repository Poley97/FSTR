...
@MIDDLE_ENCODERS.register_module()
class VoxelNextEncoderK3(VoxelNextEncoder):
    def __init__(self,
                 **kwargs):
        super(VoxelNextEncoderK3,self).__init__(**kwargs)
    ...
    def make_encoder_layers(self,
        ...
        assert block_type in ['conv_module', 'basicblock']
        self.encoder_layers = spconv.SparseSequential()
        self.sparse_conv_kernel = [5,5,3,3,3,3]
        for i, blocks in enumerate(self.encoder_channels):
            blocks_list = []
            for j, out_channels in enumerate(tuple(blocks)):
                padding = tuple(self.encoder_paddings[i])[j]
                    ...
                    if j == 0 and len(blocks) > 2:
                        blocks_list.append(
                            make_block(
                                in_channels,
                                out_channels,
                                self.sparse_conv_kernel[i],
                                norm_cfg=norm_cfg,
                                stride=2,
                                padding=int(self.sparse_conv_kernel[i]//2),
                                indice_key=f'spconv{i + 1}',
                                conv_type='SparseConv3d'))
                    else:
                        blocks_list.append(
...
@MIDDLE_ENCODERS.register_module()
class VoxelNextEncoderK5(VoxelNextEncoder):
    def __init__(self,
                 **kwargs):
        super(VoxelNextEncoderK5,self).__init__(**kwargs)
    ...
    def make_encoder_layers(self,
        ...
        assert block_type in ['conv_module', 'basicblock']
        self.encoder_layers = spconv.SparseSequential()
        self.sparse_conv_kernel = [5,5,5,3,3,3]
        for i, blocks in enumerate(self.encoder_channels):
            blocks_list = []
            for j, out_channels in enumerate(tuple(blocks)):
                padding = tuple(self.encoder_paddings[i])[j]
